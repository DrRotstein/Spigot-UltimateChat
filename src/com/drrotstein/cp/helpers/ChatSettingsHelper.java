...
public class ChatSettingsHelper {
	...
	public static void initializeItemMethods() {
		...
		MethodParameter jqmSettingsMethod = getDefaultMP("openJQM"), backToCSMethod = getDefaultMP("openCS"), antispamMethod = getDefaultMP("openAntispam"),
				...
				rmOnMethod = getDefaultMPWithString("setRM", "true"), plfMethod = getDefaultMP("openPlfEditor");
		...
		MethodParameter[] allMethods = {closeMethod, jqmSettingsMethod, joinmMethod, quitmMethod, backToCSMethod, antispamMethod, tfeMethod, slowmodeMethod, rmMethod,
				s5Method, s10Method, s15Method, s30Method, s45Method, s60Method, customSMMethod, rmOffMethod, rmOnMethod, plfMethod};
	...
	public static void openCS(Player player) {
		...
		csInv.setItem(31, playerListFormatItem);
	...
	public static void openPlfEditor(Player player) {
		new BukkitRunnable() {
			...
			public void run() {
				...
				ChatEditorManager.addEditingPlayer(player, new MethodParameter("setPlf", new Class[] {Player.class, String.class},
						new Object[] {player, null}, instance.getClass(), instance, 1));
			}
		}.runTask(UltimateChat.getPlugin());
	...
	public static void setPlf(Player player, String format) {
		ChatHelper.cmdForceMainThreadCustomSender(player, "playerlistformat set " + format);
	}
	...
	public static void setJoinmessage(Player editor, String mes) {
		ChatHelper.cmdForceMainThreadCustomSender(editor, "joinmessage set " + mes);
	}
	
	public static void setQuitmessage(Player editor, String mes) {
		ChatHelper.cmdForceMainThreadCustomSender(editor, "quitmessage set " + mes);
	}
